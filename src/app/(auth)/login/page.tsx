...
export default function LoginPage() {
  ...
  // Handle OAuth callback for native platforms
  useEffect(() => {
  ...
  const handleOAuthLogin = async (provider: 'google' | 'github') => {
    ...
    if (isNativePlatform()) {
      // On native platforms, open browser with redirect_uri pointing back to app
      const redirectUri = encodeURIComponent(getMobileRedirectUri());
      const oauthUrl = `${apiUrl}/auth/${provider}?redirect_uri=${redirectUri}`;
        ...
        <div className="card">
          ...
          {isOAuthLoading ? (
            <div className="flex items-center justify-center py-4">
              <Loader2 className="h-6 w-6 animate-spin text-pineapple" />
              <span className="ml-2 text-gray-600">Completing sign in...</span>
            </div>
          ) : (
