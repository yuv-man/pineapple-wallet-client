...
export default function EditLiabilityClient({ id }: Props) {
  ...
  useEffect(() => {
    liabilitiesApi
      ...
      .then((res) => {
        const l = res.data;
        setValue("type", l.type);
        setValue("name", l.name);
        setValue("balance", Number(l.balance));
        setValue("currency", l.currency);
        if (l.interestRate != null)
          setValue("interestRate", Number(l.interestRate) * 100);
        if (l.minimumPayment != null)
          setValue("minimumPayment", Number(l.minimumPayment));
        if (l.dueDate) setValue("dueDate", l.dueDate.slice(0, 10));
        if (l.notes) setValue("notes", l.notes);
        setSelectedType(l.type);
      })
      .catch(() => setError("Failed to load liability"))
  ...
  const onSubmit = async (data: LiabilityForm) => {
    ...
    setError(null);
    try {
      await liabilitiesApi.update(id, {
        ...data,
        interestRate:
